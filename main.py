...
lbl_title = Label(win, text='Student Management System', font=('Segoe UI', 40, 'bold'), bg='#F4F6F8', fg='#2563EB')
lbl_title.pack()
...
def footer_labels():
    footer = Frame(win, bg='#E5E7EB')
    footer.place(relx=0, rely=0.93, relwidth=1, relheight=0.07)

    lbl_email = Label(
        footer,
        text="Email: Abhishekkashyap8755679321@gmail.com",
        font=('Segoe UI', 12),
        bg='#E5E7EB'
    )
    lbl_email.pack(side=LEFT, padx=30)

    lbl_phone = Label(
        footer,
        text="Phone: +91 8755679321",
        font=('Segoe UI', 12),
        bg='#E5E7EB'
    )
    lbl_phone.pack(side=RIGHT, padx=30)
...
def add_course_db(e1, e2):
    ...
    if not re.match("^[A-Za-z +]+$", c_name):
        messagebox.showwarning('ADD COURSE', 'Course Name must contain only characters and "+".')
        return
    try:
        c_fees = int(e2.get())
    except ValueError:
...
def reg_db(e1, e2, e3, e4, e5, e6):
    ...
    if not re.match("^[A-Za-z ]+$", name):
        messagebox.showwarning('Student Reg', 'Name must contain only characters.')
        return
    mob = (e2.get())
    str_mob = str(mob)
    ...
    if not re.match("^[6-9]", str_mob[0]):
        ...
        return
    if len(mob) != 10:
        messagebox.showwarning('Student Reg', 'contact number must be of 10 character.')
        return
    email = e3.get()
    ...
    try:
        ...
        messagebox.showwarning('Student Reg', 'amount must be a numeric value')
        return
...
def registerscreen(wfrm):
    ...
    con = getcon()
    cur = con.cursor()
    cur.execute("select course_name,course_fees from courses")
    row = cur.fetchall()
    con.close()

    lbl_name = Label(frm, text='Student Name:', font=('Segoe UI', 20, 'bold'), bg='pink')
    lbl_name.place(relx=.3, rely=.1)
    entry_name = Entry(frm, font=('', 15, 'bold'), bd=5)
    entry_name.place(relx=.47, rely=.1)
    entry_name.focus()

    lbl_mob = Label(frm, text='Student Mob:', font=('Segoe UI', 20, 'bold'), bg='pink')
    lbl_mob.place(relx=.3, rely=.2)
    entry_mob = Entry(frm, font=('Segoe UI', 15, 'bold'), bd=5)
    entry_mob.place(relx=.47, rely=.2)

    lbl_email = Label(frm, text='Student Email:', font=('', 20, 'bold'), bg='pink')
    lbl_email.place(relx=.3, rely=.3)
    entry_email = Entry(frm, font=('Segoe UI', 15, 'bold'), bd=5)
    entry_email.place(relx=.47, rely=.3)

    lbl_course = Label(frm, text='Course Name:', font=('Segoe UI', 20, 'bold'), bg='pink')
    lbl_course.place(relx=.3, rely=.4)
    entry_course = ttk.Combobox(frm, textvariable=txtFrame1, state="readonly", font=('', 14, 'bold'))
    entry_course['values'] = ('', "Python", "Python+Django", "Python+ML", "Python+ML+AI")
    ...
    for count in range(lenght):
        entry_course['values'] = entry_course['values'] + (row[count][0],)
    ...
    lbl_amt = Label(frm, text='Amount:', font=('Segoe UI', 20, 'bold'), bg='pink')
    lbl_amt.place(relx=.3, rely=.6)
    entry_amt = Entry(frm, font=('Segoe UI', 15, 'bold'), bd=5)
...
def updatescreen(wfrm, sid):
    ...
    con2 = getcon()
    cur2 = con2.cursor()
    cur2.execute("select course_name,course_fees from courses")
    row2 = cur2.fetchall()
    con2.close()
    ...
    entry_course['values'] = (f'{row[2]}', "Python", "Python+Django", "Python+ML", "Python+ML+AI")
    entry_course.place(relx=.47, rely=.4)
    entry_course.current(0)
    ...
    lenght = len(row2)
    for count in range(lenght):
        entry_course['values'] = entry_course['values'] + (row2[count][0],)
    ...
    btn_update.place(relx=.4, rely=.8)
...
def welcomescreen():
    ...
    btn_course = Button(frm, text='Add course', command=lambda: addcourse(frm), font=('', 15, 'bold'), bd=5, width=18)
    btn_course.place(relx=.4, y=370)
...
def homescreen():
    ...
    lbl_credits = Label(frm, text='Aayush Kumar Mishra', font=('', 18, 'bold'), bg='pink', fg='red')
    lbl_credits.place(x=1150, y=570)

    lbl_email = Label(frm, text='📧 Email: your.email@example.com', font=('', 12, 'bold'), bg='pink', fg='#333333')
    lbl_email.place(x=1100, y=600)

    lbl_phone = Label(frm, text='📱 Phone: +91-XXXXXXXXXX', font=('', 12, 'bold'), bg='pink', fg='#333333')
    lbl_phone.place(x=1100, y=630)
